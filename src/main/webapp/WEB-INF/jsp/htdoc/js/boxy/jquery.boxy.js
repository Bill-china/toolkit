...
function Boxy(element, options) {
    ...
    if (this.options.modal) {
        this.options = $.extend(this.options, {center: true});
    }
...
$.extend(Boxy, {
    ...
    WRAPPER:    "<table cellspacing='0' cellpadding='0' border='0' class='boxy-wrapper'>" +
                ...
                "<tr><td class='boxy-left'></td><td><div class='boxy-inner'></div></td><td class='boxy-right'></td></tr>" +
    ...
    ask: function(question, answers, callback, options) {
        ...
        var body = $('<div></div>').append($('<div class="boxy-question"></div>').html(question));
        ...
        var buttons = $('<form class="boxy-answers"></form>');
        buttons.html($.map(answerStrings, function(v) {
            return "<button type='button' value='" + v + "' class='rst_btn'><span class='btn'><span>" + v + "</span></span></button>";
        }).join(' '));
        
        $('button', buttons).click(function() {
        	var clicked = $(".btn span",this);
...
Boxy.prototype = {
    ...
    show: function() {
        ...
        if (this.options.modal) {
            ...
            if (!Boxy.resizeConfigured) {
                ...
                $(window).resize(function() { Boxy._handleResize(); });
            }
            // huchengquan fix ie6 8,pageSize 2010-12-22
            var d = document,
                p = d.compatMode === 'CSS1Compat' ? d.documentElement : d.body,
                s = { width : p.scrollWidth, height : p.scrollHeight };
            
            this.modalBlackout = $('<iframe frameborder="0" class="boxy-modal-iframe"></iframe><div class="boxy-modal-blackout"></div>')
                .css({zIndex: Boxy._nextZ(),
                    opacity: 0.3,
                    width: s.width,//$(document).width(),
                    height: s.height})//$(document).height()})
        ...
        this.visible = true;
		this.getContent().find("button:first").focus();
    ...
    _setupTitleBar: function() {
        if (this.options.title) {
            var self = this;
            var tb = $("<div class='boxy-title-bar'></div>").html("<h2 class='bartool'>" + this.options.title + "</h2>");
            if (this.options.closeable) {
                tb.append($("<a href='javascript:;' class='boxy-close'></a>").html(this.options.closeText));
            }
            if (this.options.draggable) {
                ...
                tb.mousedown(function(evt) {
                    ...
                    $(this).addClass('boxy-dragging');
                }).mouseup(function() {
                    $(this).removeClass('boxy-dragging');
                    Boxy.dragging = null;
    ...
    _setupDefaultBehaviours: function(root) {
        ...
        $('.boxy-close', root).click(function() {
            self.hide();
            return false;

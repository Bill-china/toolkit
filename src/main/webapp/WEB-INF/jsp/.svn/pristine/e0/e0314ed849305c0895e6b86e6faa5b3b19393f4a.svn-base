...
class Interest extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('time, key, group_id, user_id, plan_id, inter_id, click_times, view_times, total_cost, trans_times, stats_date', 'required'),
			array('group_id, user_id, plan_id, click_times, view_times, trans_times, status', 'numerical', 'integerOnly'=>true),
			array('total_cost', 'numerical'),
			array('key, inter_id', 'length', 'max'=>64),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, time, key, group_id, user_id, plan_id, inter_id, click_times, view_times, total_cost, trans_times, stats_date, status', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		return new CActiveDataProvider('Interest', array(
			'criteria'=>$criteria,
		));
	}
